...
- `scripts/check-demo-language.mjs` — runs the shipped `assets/portfolio-destination.js` against the served homepage to assert every demo card resolves to `?lang=zh` or `?lang=en` with the active portfolio language, and that the homepage still routes its destinations through that resolver
